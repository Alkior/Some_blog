...
class ArticleController extends BaseController
{
	...
	public function addAction()
	{
		...
		if(count($this->request->post) > 0 ){
			$errors = $mArticle->isValidate($this->request->getPost()['title'], $this->request->getPost()['title']);
			if(empty($errors)){

				$article = $mArticle->write(htmlspecialchars($this->request->getPost()['title']), htmlspecialchars($this->request->getPost()['title']));
				header('Location: ../');
			}
		}
